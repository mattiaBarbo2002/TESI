...
def pretrain_encoders(
    ...
    try:
        for epoch in range(1, epochs + 1):
            ...
            if epoch_loss < best_loss:
                ...
                torch.save(state_dict, f'modelS2_best_{job_name}.pth')
                best_loss = epoch_loss
                epochs_no_improve = 0
    ...
    try:
        pd.DataFrame(resultsS2).to_csv(f'log_pretrainS2_{job_name}.csv', index_label='epoch')
        project_work.log_artifact(name=f"encoder-s2-weights_{job_name}", source=f"modelS2_best_{job_name}.pth")
        project_work.log_artifact(name=f"metrics-s2_{job_name}", source=f"log_pretrainS2_{job_name}.csv")
...
def train_moco(
    ...
    workers: int = 0,
    job_name: str = "nome_job",
    encoders_job_name: str = "nome_encoders_job",
    dataset: str = "Test"
):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print('Using device:', device, "\n", flush=True)
    ...
    path_s1 = project_work.get_artifact(f"encoder-s1-weights_{encoders_job_name}").download(f"modelS1_best_{encoders_job_name}.pth")
    path_s2 = project_work.get_artifact(f"encoder-s2-weights_{encoders_job_name}").download(f"modelS2_best_{encoders_job_name}.pth")
    ...
    try:
        for epoch in range(1, epochs + 1):
            ...
            if epoch_loss < best_loss:
                torch.save(model.state_dict(), f'moco_best_{job_name}.pth')
                best_loss = epoch_loss

    except Exception as e:
        print(f"EXC -> Eccezione in MoCo train: {e}", flush=True)

    pd.DataFrame(results).to_csv(f'log_moco_{job_name}.csv', index_label='epoch')

    try:
        project_work.log_artifact(name=f"moco-weights_{job_name}", source=f"moco_best_{job_name}.pth")
        project_work.log_artifact(name=f"moco-metrics_{job_name}", source=f"log_moco_{job_name}.csv")
