...
class Singlemodal_Encoder_2d(nn.Module):
    def __init__(self, input_dim=2, output_dim=16, n_images=4, n_head=8, d_k=8):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(input_dim, 16, kernel_size=(1, 7, 7), padding='same', stride=(1, 1, 1)),
            nn.BatchNorm3d(16),
            nn.ReLU()
        )

        # stage 1-2: ridimensione spaziale H e W (256 -> 128 -> 64)
        self.stage1 = nn.Sequential(ConvBlock(16, kernel_size=(1, 3, 3), filters=[16, 16, 64], strides=(1, 2, 2)),
                                     CBAM(n_images, reduction_ratio=1))
        self.stage2 = nn.Sequential(ConvBlock(64, kernel_size=(1, 3, 3), filters=[16, 16, 64], strides=(1, 2, 2)),
                                     CBAM(n_images, reduction_ratio=1))

        # stage 3-6: dim spaziale rimane 64x64, i canali aumentano a 256
...
class Singlemodal_CAE_2d(nn.Module):
    def __init__(self, input_dim=2, output_dim=16, n_images=4, n_head=8, d_k=8):
        ...
        # lstm dovrebbe imparare evoluzione temporale e stagionale
        self.conv_lstm = ConvLSTM(input_dim=output_dim, hidden_dim=256, kernel_size=(3, 3),
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1, 3, 4)                                # (batch, 256, 4, 64, 64)
        x = self.stage1(x)
        x = self.stage2(x)
        x = self.stage3(x)
        x = self.stage4(x)
