...
NODATA_VALUE = -9999
FIXED_CEILING = 3000          # soglia "colore naturale" per S2 SR
FRAZ_OLTRE_SOGLIA_MAX = 0.5

CSV_PATH = os.path.join(PROJECT_ROOT, "csv", "complete_series.csv")

# cartelle
S1_FOLDER = os.path.join(PROJECT_ROOT, "data", "dataset", "SAR")
S2_FOLDER = os.path.join(PROJECT_ROOT, "data", "dataset", "OPT")
...
def normalize_rgb_percentile(rgb_stack, mask_invalid):
    pixel_validi = rgb_stack[~mask_invalid]
    if pixel_validi.size == 0:
        return np.zeros_like(rgb_stack)
    p2, p98 = np.percentile(pixel_validi, [2, 98])  # una sola chiamata
    if p98 <= p2:
        return np.zeros_like(rgb_stack)
    return np.clip((rgb_stack - p2) / (p98 - p2), 0, 1)

def normalize_rgb(rgb_stack, mask_invalid, fixed_ceiling=FIXED_CEILING, soglia_frazione=FRAZ_OLTRE_SOGLIA_MAX):
    valid = rgb_stack[~mask_invalid]
    if valid.size == 0:
        return np.zeros_like(rgb_stack), False

    frazione_satura = np.mean(valid > fixed_ceiling)

    if frazione_satura < soglia_frazione:
        rgb_norm = np.clip(rgb_stack, 0, fixed_ceiling) / fixed_ceiling
        return rgb_norm, False
    else:
        p2, p98 = np.percentile(valid, [2, 98])
        if p98 <= p2:
            return np.zeros_like(rgb_stack), True
        rgb_norm = np.clip((rgb_stack - p2) / (p98 - p2), 0, 1)
        return rgb_norm, True    
        ...
        for step in range(4):
            ...
            if paths_sar[step]:
                with rasterio.open(paths_sar[step]) as src:
                    ...
                    # Maschera: NaN veri, sentinella -9999 usata in download, ed eventuale nodata del file
                    mask_nan_sar = np.isnan(img) | (img == NODATA_VALUE)
                    if src.nodata is not None:
                        mask_nan_sar |= (img == src.nodata)

                    img[mask_nan_sar] = np.nan

                    # Disegna i NaN del SAR in Magenta
            ...
            if paths_opt[step]:
                with rasterio.open(paths_opt[step]) as src:
                    ...
                    mask_nan_opt = (
                        np.isnan(r_raw) | np.isnan(g_raw) | np.isnan(b_raw) |
                        (r_raw == NODATA_VALUE) | (g_raw == NODATA_VALUE) | (b_raw == NODATA_VALUE)
                    )
                    if src.nodata is not None:
                        mask_nan_opt |= (r_raw == src.nodata) | (g_raw == src.nodata) | (b_raw == src.nodata)

                    rgb_stack = np.dstack((r_raw, g_raw, b_raw))
                    rgb_norm, auto_stretch = normalize_rgb(rgb_stack, mask_nan_opt)
                    rgb_gamma = np.power(rgb_norm, 0.75)
                    rgb_gamma[mask_nan_opt] = [1.0, 0.0, 1.0]

                    ax_opt.imshow(rgb_gamma)
            titolo_opt = f"OPT t{step+1}\n{date_opt[step]}"
            ax_opt.set_title(titolo_opt)
            ax_opt.axis('off')
    ...
    try:
        ...
        if successo:
            ...
            if os.path.exists(percorso_finale_zip):
                ...
                print(f"🔴 ELIMINATO: {nome_file_out}")
            return None
...
def processa_singola_serie(riga, genera_png, riga_num):
    ...
    if genera_png:
        nome_output_png = os.path.join(BASE_PNG_FOLDER, f"{riga_num}_{nome_serie}.png")
...
if __name__ == "__main__":
    ...
    for dati in risultati_per_png:
        print(f"Disegno PNG per: {dati['nome_serie']}")
